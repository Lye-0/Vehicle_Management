...
function serializeMaintenanceDocument(document: typeof maintenanceDocuments.$inferSelect, customer: typeof customers.$inferSelect | undefined, vehicle: typeof vehicles.$inferSelect | undefined, rows: Array<typeof maintenanceItems.$inferSelect>) {
  ...
  return {
    ...
    vehicleDetails: {
      maker: vehicle?.maker ?? '',
      name: vehicle?.name ?? '',
      modelYear: vehicle?.modelYear === null || vehicle?.modelYear === undefined ? '' : String(vehicle.modelYear),
      ...
      inspectionExpiryDate: vehicle?.inspectionDate ?? '',
      hasMaintenanceRecord: vehicle?.inspectionRecordAvailable ?? false,
    },
...
function parseMaintenanceDetails(value: unknown): MaintenanceDetails {
  ...
  return {
    ...
    vehicleOverride: {
      ...
      modelYear: stringValue(vehicleOverride, 'modelYear'),
      displacement: stringValue(vehicleOverride, 'displacement'),
      transmission: stringValue(vehicleOverride, 'transmission'),
      bodyColor: stringValue(vehicleOverride, 'bodyColor'),
      modelCode: stringValue(vehicleOverride, 'modelCode'),
      chassisNumber: stringValue(vehicleOverride, 'chassisNumber'),
      registrationNumber: stringValue(vehicleOverride, 'registrationNumber'),
      mileage: stringValue(vehicleOverride, 'mileage'),
      inspectionExpiryDate: stringValue(vehicleOverride, 'inspectionExpiryDate'),
      hasMaintenanceRecord: typeof vehicleOverride.hasMaintenanceRecord === 'boolean' ? vehicleOverride.hasMaintenanceRecord : false,
    },
    labels: {
