...
export function CustomerVehiclePage({ onNavigate, initialCustomerId, initialVehicleId, onNavigationConsumed }: { onNavigate?: (target: VehicleHistoryNavigation) => void; initialCustomerId?: string; initialVehicleId?: string; onNavigationConsumed?: () => void } = {}) {
  ...
  return (
    <>
      ...
      {customerDialogOpen && <CustomerDialog form={customerForm} title={editingCustomerId ? '顧客情報を編集' : '顧客を登録'} submitLabel={editingCustomerId ? '変更を保存' : '顧客を登録'} cancelLabel={editingCustomerId ? '閉じる' : undefined} autosaveStatus={customerAutosave.status} autosaveLastSavedAt={customerAutosave.lastSavedAt} onChange={updateCustomerForm} onClose={closeCustomerDialog} onSubmit={handleCustomerSubmit} />}
      {vehicleDialogOpen && selectedCustomer && <VehicleDialog form={vehicleForm} title={editingVehicleId ? '車両情報を編集' : '車両を追加'} submitLabel={editingVehicleId ? '変更を保存' : '車両を追加'} cancelLabel={editingVehicleId ? '閉じる' : undefined} autosaveStatus={vehicleAutosave.status} autosaveLastSavedAt={vehicleAutosave.lastSavedAt} customerName={selectedCustomer.name} onChange={updateVehicleForm} onClose={closeVehicleDialog} onSubmit={handleVehicleSubmit} />}
...
function CustomerDialog({ form, title, submitLabel, cancelLabel, autosaveStatus, autosaveLastSavedAt, onChange, onClose, onSubmit }: { form: CustomerInput; title: string; submitLabel: string; cancelLabel?: string; autosaveStatus: AutosaveState; autosaveLastSavedAt: number | null; onChange: (form: CustomerInput) => void; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
...
function VehicleDialog({ form, title, submitLabel, cancelLabel, autosaveStatus, autosaveLastSavedAt, customerName, onChange, onClose, onSubmit }: { form: VehicleInput; title: string; submitLabel: string; cancelLabel?: string; autosaveStatus: AutosaveState; autosaveLastSavedAt: number | null; customerName: string; onChange: (form: VehicleInput) => void; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
