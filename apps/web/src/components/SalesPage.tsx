...
function EstimateVehicleTable({ vehicle }: { vehicle: SalesDocument['vehicleDetails'] }) {
  ...
  return <section className="sales-estimate-vehicle-table"><div className="sales-estimate-vehicle-row sales-estimate-vehicle-labels"><span>メーカー</span><span>車名・仕様</span><span>年式</span><span>排気量</span><span>ミッション</span><span>車体色</span></div><div className="sales-estimate-vehicle-row"><span>{values.maker || '未設定'}</span><span>{values.name || '未設定'}{values.modelType ? ` / ${values.modelType}` : ''}</span><span>{values.year || '未設定'}</span><span>{values.displacement || '未設定'}</span><span>{values.transmission || '未設定'}</span><span>{values.color || '未設定'}</span></div><div className="sales-estimate-vehicle-row sales-estimate-vehicle-labels"><span>型式</span><span>車台番号</span><span>登録番号</span><span>走行距離</span><span>車検日</span><span>記録簿</span></div><div className="sales-estimate-vehicle-row"><span>{values.modelType || '未設定'}</span><span>{values.vin || '未設定'}</span><span>{values.plate || '未設定'}</span><span>{values.mileage || '未設定'}</span><span>{values.inspectionDate || '未設定'}</span><span>{values.inspectionRecordAvailable ? 'あり' : 'なし'}</span></div></section>
}
