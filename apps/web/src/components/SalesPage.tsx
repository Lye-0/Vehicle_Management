...
import { DocumentTaxSettings, type TaxRounding } from './DocumentTaxSettings'
...
export function SalesPage({ initialDocumentId }: { initialDocumentId?: string } = {}) {
  ...
  function updateTaxRounding(value: TaxRounding) {
    if (!selectedDocument) return
    replaceDocuments((current) => current.map((document) => document.id === selectedDocument.id ? { ...document, taxRounding: value } : document))
    markDirty()
  }
...
function SalesDocumentEditor(props: { document: SalesDocument; totals: SalesTotals; itemPresets: string[]; customers: Customer[]; onUpdateHeader: (field: SalesHeaderField, value: string) => void; onUpdateDetails: (patch: Partial<SalesDocumentDetails>) => void; onUpdateTaxRate: (value: number) => void; onUpdateTaxRounding: (value: TaxRounding) => void; onUpdateTradeIn: (field: keyof SalesDocumentDetails['tradeIn'], value: string) => void; onUpdateCredit: (field: keyof SalesDocumentDetails['credit'], value: string | boolean) => void; onUpdateRequiredDocument: (field: keyof SalesDocumentDetails['requiredDocuments'], value: string | boolean) => void; onUpdateItem: (itemId: string, field: SalesItemField, value: string) => void; onAddItem: () => void; onRemoveItem: (itemId: string) => void }) {
  ...
  return <>
    ...
    <details className="sales-details-accordion">
      ...
      <div className="sales-details-accordion-content"><DocumentTaxSettings taxRate={Math.round(document.taxRate * 100)} taxRounding={document.taxRounding} onTaxRateChange={props.onUpdateTaxRate} onTaxRoundingChange={props.onUpdateTaxRounding} /></div>
    </details>
  </>
}
