...
function MaintenanceDocumentDetail({ document, customers, totals, shopName, itemPresets, view, saving, saved, onViewChange, onUpdateHeader, onSave, onArchive, onPdfDownload, onPdfPreview, onUpdateItem, onAddItem, onRemoveItem, onUpdateFee, onUpdateAdjustment }: { document: MaintenanceDocument; customers: Customer[]; totals: MaintenanceTotals; shopName: string; itemPresets: string[]; view: MaintenanceDocumentView; saving: boolean; saved: boolean; onViewChange: (view: MaintenanceDocumentView) => void; onUpdateHeader: (field: MaintenanceHeaderField, value: string) => void; onSave: () => void; onArchive: () => void; onPdfDownload: () => void; onPdfPreview: () => void; onUpdateItem: (itemId: string, field: 'kind' | 'description' | 'quantity' | 'unitPrice', value: string) => void; onAddItem: () => void; onRemoveItem: (itemId: string) => void; onUpdateFee: (key: keyof MandatoryFees, value: string) => void; onUpdateAdjustment: (value: string) => void }) {
  return <section className="panel maintenance-detail-panel"><div className="maintenance-detail-header"><div className="maintenance-detail-title"><div><span className={`maintenance-category-badge maintenance-category-${document.category}`}>{document.category}</span><h2>{document.number}</h2><small>{document.type} ・ 発行元 {shopName}</small></div><MaintenanceStatusTag status={document.status} /></div><div className="maintenance-detail-actions"><button className="button button-secondary" type="button" onClick={onPdfPreview}><Eye size={16} />PDFで確認</button><button className="button button-secondary" type="button" disabled={saving} onClick={onSave}><Save size={16} />{saving ? '保存中…' : saved ? '保存済み' : '保存'}</button><button className="button button-secondary" type="button" onClick={onPdfDownload}><FileDown size={16} />PDF保存</button><button className="button button-danger" type="button" disabled={saving} onClick={onArchive}><Archive size={16} />アーカイブ</button></div></div><div className="maintenance-document-tabs" role="tablist" aria-label="整備書類の表示"><button id="maintenance-document-edit-tab" className={view === 'edit' ? 'is-active' : ''} type="button" role="tab" aria-selected={view === 'edit'} aria-controls="maintenance-document-edit-panel" onClick={() => onViewChange('edit')}><FileText size={16} />入力</button><button id="maintenance-document-preview-tab" className={view === 'preview' ? 'is-active' : ''} type="button" role="tab" aria-selected={view === 'preview'} aria-controls="maintenance-document-preview-panel" onClick={() => onViewChange('preview')}><Eye size={16} />プレビュー</button></div>{view === 'edit' ? <div id="maintenance-document-edit-panel" className="maintenance-detail-content" role="tabpanel" aria-labelledby="maintenance-document-edit-tab"><MaintenanceDocumentEditor document={document} customers={customers} totals={totals} itemPresets={itemPresets} onUpdateHeader={onUpdateHeader} onUpdateItem={onUpdateItem} onAddItem={onAddItem} onRemoveItem={onRemoveItem} onUpdateFee={onUpdateFee} onUpdateAdjustment={onUpdateAdjustment} /></div> : <div id="maintenance-document-preview-panel" className="maintenance-detail-content" role="tabpanel" aria-labelledby="maintenance-document-preview-tab"><MaintenancePreviewPlaceholder /></div>}</section>
}

function MaintenanceDocumentEditor({ document, customers, totals, itemPresets, onUpdateHeader, onUpdateItem, onAddItem, onRemoveItem, onUpdateFee, onUpdateAdjustment }: { document: MaintenanceDocument; customers: Customer[]; totals: MaintenanceTotals; itemPresets: string[]; onUpdateHeader: (field: MaintenanceHeaderField, value: string) => void; onUpdateItem: (itemId: string, field: 'kind' | 'description' | 'quantity' | 'unitPrice', value: string) => void; onAddItem: () => void; onRemoveItem: (itemId: string) => void; onUpdateFee: (key: keyof MandatoryFees, value: string) => void; onUpdateAdjustment: (value: string) => void }) {
  ...
  return <>
    ...
    <details className="maintenance-details-accordion"><summary><span>詳細設定</span><ChevronDown size={16} aria-hidden="true" /></summary><div className="maintenance-details-accordion-content"><MaintenanceDetailedFields document={document} totals={totals} itemPresets={itemPresets} onUpdateHeader={onUpdateHeader} onUpdateItem={onUpdateItem} onAddItem={onAddItem} onRemoveItem={onRemoveItem} onUpdateFee={onUpdateFee} onUpdateAdjustment={onUpdateAdjustment} /></div></details>
  </>
}

function MaintenanceDetailedFields({ document, totals, itemPresets, onUpdateHeader, onUpdateItem, onAddItem, onRemoveItem, onUpdateFee, onUpdateAdjustment }: { document: MaintenanceDocument; totals: MaintenanceTotals; itemPresets: string[]; onUpdateHeader: (field: MaintenanceHeaderField, value: string) => void; onUpdateItem: (itemId: string, field: 'kind' | 'description' | 'quantity' | 'unitPrice', value: string) => void; onAddItem: () => void; onRemoveItem: (itemId: string) => void; onUpdateFee: (key: keyof MandatoryFees, value: string) => void; onUpdateAdjustment: (value: string) => void }) {
  return <div className="maintenance-detail-fields"><div className="maintenance-context-grid"><div className="maintenance-context-card"><span className="maintenance-context-label"><UserRound size={15} />顧客</span><strong>{document.customerName}</strong><small>{document.phone}</small></div><div className="maintenance-context-card"><span className="maintenance-context-label"><CarFront size={15} />対象車両</span><strong>{document.vehicle || '車両未指定'}</strong><small>{document.plate || '登録番号未登録'} ・ {document.mileage || '走行距離未登録'}</small></div></div><section className="maintenance-items-panel"><div className="maintenance-items-header"><div><h3>作業・部品明細</h3><span>作業内容や部品名を明細行として登録します</span></div><button className="text-button" type="button" onClick={onAddItem}><Plus size={15} />明細を追加</button></div><div className="maintenance-items-table"><div className="maintenance-items-head"><span>区分</span><span>作業内容・部品名</span><span>数量</span><span>単位</span><span>単価</span><span>金額</span><span /></div>{document.items.map((item) => <div className="maintenance-item-row" key={item.id}><select aria-label="明細区分" value={item.kind} onChange={(event) => onUpdateItem(item.id, 'kind', event.target.value)}><option>作業</option><option>部品</option></select><input list="maintenance-item-presets" aria-label="作業内容・部品名" value={item.description} onChange={(event) => onUpdateItem(item.id, 'description', event.target.value)} placeholder="作業内容・部品名" /><input className="maintenance-number-input" aria-label="数量" type="number" min="0" value={item.quantity} onChange={(event) => onUpdateItem(item.id, 'quantity', event.target.value)} /><span>{item.unit}</span><input className="maintenance-price-input" aria-label="単価" type="number" value={item.unitPrice} onChange={(event) => onUpdateItem(item.id, 'unitPrice', event.target.value)} /><strong>{formatYen(item.quantity * item.unitPrice)}</strong><button className="maintenance-item-remove" type="button" aria-label="明細を削除" onClick={() => onRemoveItem(item.id)}><Trash2 size={15} /></button></div>)}</div><datalist id="maintenance-item-presets">{itemPresets.map((preset) => <option key={preset} value={preset} />)}</datalist></section><section className="maintenance-fees-panel"><div className="maintenance-fees-header"><div><h3>税金・保険料・その他</h3><span>車検時の法定費用やリサイクル料金を登録します</span></div><CircleDollarSign size={19} /></div><div className="maintenance-fees-grid"><FeeField label="自賠責保険" value={document.fees.自賠責} onChange={(value) => onUpdateFee('自賠責', value)} /><FeeField label="重量税" value={document.fees.重量税} onChange={(value) => onUpdateFee('重量税', value)} /><FeeField label="印紙代" value={document.fees.印紙代} onChange={(value) => onUpdateFee('印紙代', value)} /><FeeField label="リサイクル料金" value={document.fees.リサイクル料金} onChange={(value) => onUpdateFee('リサイクル料金', value)} /></div></section><div className="maintenance-summary-grid"><label className="maintenance-note maintenance-note-editor"><span>備考</span><textarea aria-label="備考" value={document.note} onChange={(event) => onUpdateHeader('note', event.target.value)} placeholder="備考を入力" /></label><div className="maintenance-totals"><div><span>作業・部品小計</span><strong>{formatYen(totals.itemsSubtotal)}</strong></div><div><span>法定費用等</span><strong>{formatYen(totals.fees)}</strong></div><div><label htmlFor="maintenance-adjustment">調整額</label><input id="maintenance-adjustment" type="number" value={document.adjustment} onChange={(event) => onUpdateAdjustment(event.target.value)} /></div><div><span>消費税（{document.taxRate * 100}%）</span><strong>{formatYen(totals.tax)}</strong></div><div className="maintenance-total-row"><span>合計金額</span><strong>{formatYen(totals.total)}</strong></div></div></div></div>
}

function MaintenancePreviewPlaceholder() {
  return <div className="maintenance-preview-placeholder" aria-label="整備書類プレビュー（準備中）"><span className="sr-only">整備書類プレビューは準備中です。</span></div>
}
