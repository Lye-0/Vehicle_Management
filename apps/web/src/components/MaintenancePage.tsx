...
type MaintenanceCreateForm = { type: MaintenanceDocumentType; category: IntakeCategory; customerId: string; vehicleId: string; intakeDate: string; plannedReleaseDate: string }

const emptyFees: MandatoryFees = { 自賠責: 0, 重量税: 0, 印紙代: 0, リサイクル料金: 0 }
const emptyCreateForm: MaintenanceCreateForm = { type: '整備見積書', category: '一般整備', customerId: '', vehicleId: '', intakeDate: todayDisplay(), plannedReleaseDate: addDaysDisplay(2) }
...
export function MaintenancePage() {
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([fetchMaintenanceDocuments(), fetchCustomers()])
      .then(([nextDocuments, nextCustomers]) => {
        if (cancelled) return
        setDocuments(nextDocuments)
        setCustomers(nextCustomers)
        setSelectedDocumentId(nextDocuments[0]?.id ?? '')
        setCreateForm(createFormForCustomers(nextCustomers))
        setError('')
      })
  ...
  async function createDocument(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setCreateForm(createFormForCustomers(customers))
...
function MaintenanceDocumentDetail({ document, totals, saving, saved, onSave, onPdf, onUpdateItem, onAddItem, onRemoveItem, onUpdateFee, onUpdateAdjustment }: { document: MaintenanceDocument; totals: MaintenanceTotals; saving: boolean; saved: boolean; onSave: () => void; onPdf: () => void; onUpdateItem: (itemId: string, field: 'kind' | 'description' | 'quantity' | 'unitPrice', value: string) => void; onAddItem: () => void; onRemoveItem: (itemId: string) => void; onUpdateFee: (key: keyof MandatoryFees, value: string) => void; onUpdateAdjustment: (value: string) => void }) {
...
function calculateMaintenanceTotals(document: MaintenanceDocument): MaintenanceTotals { const itemsSubtotal = document.items.reduce((sum, item) => sum + item.quantity * item.unitPrice, 0); const fees = Object.values(document.fees).reduce((sum, fee) => sum + fee, 0); const taxableAmount = Math.max(0, itemsSubtotal + document.adjustment); const tax = Math.floor(taxableAmount * document.taxRate); return { itemsSubtotal, fees, tax, total: itemsSubtotal + fees + document.adjustment + tax } }
...
function toMaintenanceInput(document: MaintenanceDocument): MaintenanceDocumentInput { return { type: document.type, status: document.status, category: document.category, customerId: document.customerId, vehicleId: document.vehicleId, intakeDate: document.intakeDate, plannedReleaseDate: document.plannedReleaseDate, taxRate: document.taxRate, fees: document.fees, adjustment: document.adjustment, note: document.note, items: document.items.map(({ id: _id, ...item }) => item) } }
function createFormForCustomers(customers: Customer[]): MaintenanceCreateForm { const customer = customers[0]; return { ...emptyCreateForm, customerId: customer?.id ?? '', vehicleId: customer?.vehicles[0]?.id ?? '' } }
