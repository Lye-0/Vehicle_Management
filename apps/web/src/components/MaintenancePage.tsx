...
export function MaintenancePage({ initialDocumentId }: { initialDocumentId?: string } = {}) {
  ...
  function updateTaxRounding(value: TaxRounding) {
    if (!selectedDocument) return
    setDocuments((current) => current.map((document) => document.id === selectedDocument.id ? { ...document, taxRounding: value } : document))
    setSavedDocumentId('')
  }
...
function MaintenanceDocumentDetail({ document, customers, settings, itemPresets, view, saving, saved, onViewChange, onUpdateHeader, onUpdateDetails, onUpdateTaxRate, onUpdateTaxRounding, onSave, onArchive, onPdfDownload, onPdfPreview, onUpdateItem, onAddItem, onRemoveItem, onUpdateFee }: MaintenanceDocumentDetailProps) {
...
type MaintenanceDocumentDetailProps = {
  ...
  onUpdateTaxRounding: (value: TaxRounding) => void
...
function MaintenanceDocumentEditor({ document, customers, onUpdateHeader, onUpdateTaxRate, onUpdateTaxRounding }: { document: MaintenanceDocument; customers: Customer[]; onUpdateHeader: (field: MaintenanceHeaderField, value: string) => void; onUpdateTaxRate: (value: number) => void; onUpdateTaxRounding: (value: TaxRounding) => void }) {
  ...
  return <>
    ...
    <details className="maintenance-details-accordion"><summary><span>詳細設定</span><ChevronDown size={16} aria-hidden="true" /></summary><div className="maintenance-details-accordion-content"><DocumentTaxSettings taxRate={Math.round(document.taxRate * 100)} taxRounding={document.taxRounding} onTaxRateChange={onUpdateTaxRate} onTaxRoundingChange={onUpdateTaxRounding} /></div></details>
  </>
}
