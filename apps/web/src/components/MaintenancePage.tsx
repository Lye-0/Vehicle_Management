...
export function MaintenancePage({ initialDocumentId }: { initialDocumentId?: string } = {}) {
  ...
  const documentsRef = useRef<MaintenanceDocument[]>([])
  ...
  const selectedDocument: MaintenanceDocumentLike | null = draftDocument ?? (selectedPersistedDocument?.isSummary ? null : selectedPersistedDocument)
  ...
  function replaceActiveDocument(updater: (document: MaintenanceDocumentLike) => MaintenanceDocumentLike) {
    ...
    replaceDocuments((current) => current.map((document) => document.id === selectedPersistedDocument.id ? updater(document) as MaintenanceDocument : document))
  }
  ...
  function updateHeader(field: 'number' | 'type' | 'status' | 'category' | 'customerId' | 'vehicleId' | 'intakeDate' | 'plannedReleaseDate' | 'issuedAt' | 'dueDate' | 'note', value: string) {
    ...
    if (draftDocument) {
      ...
      replaceDocuments((current) => current.map((document) => document.id !== selectedDocument.id ? document : updateMaintenanceHeader(document, field, value, customers)))
    }
    markChanged()
  }
  ...
  async function saveSelectedDocument(mileageSync?: MaintenanceMileageSync, masterSync?: MaintenanceMasterSync, snapshot?: MaintenanceDocument): Promise<boolean> {
    if (draftDocument || !selectedPersistedDocument || saving) return false
    setSaving(true)
    setError('')
    try {
      const documentToSave = snapshot?.id === selectedPersistedDocument.id ? snapshot : selectedPersistedDocument
      const input = toMaintenanceInput(documentToSave, mileageSync)
      input.expectedUpdatedAt = documentToSave.updatedAt
  ...
  async function createMaintenanceDraftDocument(document: MaintenanceDocumentLike, context: MaintenanceDraftContext, duplicateConfirmation?: MaintenanceDuplicateConfirmation, masterSync?: MaintenanceMasterSync, mileageSync?: MaintenanceMileageSync) {
    ...
    try {
      ...
      replaceDocuments((current) => [saved, ...current.filter((document) => document.id !== saved.id)])
...
function AbacusMigrationNotice({ document }: { document: MaintenanceDocumentLike }) {
  return <div className="abacus-detail-notice" role="status">ABACUS移行明細：元データの未入力項目は空欄で表示しています。{document.abacusDetailReport?.amountOnlyRowCount ? ` 金額のみの行 ${document.abacusDetailReport.amountOnlyRowCount}件。` : ''}{document.abacusDetailReport?.warning ? ` ${document.abacusDetailReport.warning}` : ''}</div>
}
