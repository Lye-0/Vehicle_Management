...
export type MaintenanceDuplicateDialogState =
...
type Props = {
  state: MaintenanceDuplicateDialogState
...
export function MaintenanceDuplicateConfirmationDialog({
  state,
  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onCancel() }}>
      <section className="modal master-sync-modal" role="dialog" aria-modal="true" aria-labelledby="maintenance-duplicate-dialog-title" style={{ maxWidth: 560 }}>
        <div className="modal-header">
          <h2 id="maintenance-duplicate-dialog-title">{state.kind === 'customer' ? '顧客の重複確認' : state.matchReason === 'chassis_number' ? '車台番号の重複確認' : '登録番号の重複確認'}</h2>
