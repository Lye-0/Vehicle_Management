...
const vehicleInspectionLegendCategories: Array<{ category: DashboardCalendarEvent['category']; label: string }> = [{ category: 'vehicle-inspection', label: '車検満了' }]
...
export function InspectionSchedulesPage({ initialScheduleId }: { initialScheduleId?: string } = {}) {
  ...
  const vehicleInspectionEvents = useMemo<DashboardCalendarEvent[]>(() => customers.flatMap((customer) => customer.vehicles.flatMap((vehicle) => {
    if (!vehicle.inspectionDate) return []
    const customerName = customer.name || '顧客未登録'
    const vehicleName = [vehicle.maker, vehicle.model].filter(Boolean).join(' ') || '車両未登録'
    ...
    return [{
      ...
      categoryLabel: '車検満了',
      title: `車検満了：${customerName}`,
      detail: `${vehicleName}${plate}`,
      status: '車検満了',
      amount: null,
    }]
  })), [customers])
