...
export function BackupSettingsPanel() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setManualDestination(settingsResponse.settings.destination)
      setPeriodicOpen(settingsResponse.settings.autoEnabled)
      setError('')
  ...
  async function runSaveSettings() {
    ...
    setMessage('')
    try {
      const next = await updateBackupSettings({ ...backupSettings, destination: 'b2' })
  ...
  const manualDestinationLabel = manualDestination === 'b2' ? 'オンライン（B2）' : manualDestination === 'pc' ? 'PC' : '両方'
  ...
  return <section className="panel settings-panel backup-panel">
    ...
    <section className="backup-accordion">
      <div className="backup-accordion-header">
        ...
        <label className="backup-toggle"><span className="sr-only">定期バックアップを有効にする</span><input type="checkbox" checked={backupSettings.autoEnabled} disabled={!canManage || Boolean(loading)} onChange={(event) => { setBackupSettings((current) => ({ ...current, autoEnabled: event.target.checked })); setPeriodicOpen(event.target.checked) }} /><span className="backup-toggle-track" aria-hidden="true"><span /></span></label>
      </div>
    ...
    <section className="backup-accordion">
      <button className="backup-accordion-trigger backup-accordion-trigger-full" type="button" aria-expanded={manualOpen} aria-controls="manual-backup-settings" onClick={() => setManualOpen((open) => !open)}><span className="backup-accordion-heading"><strong>今すぐ手動バックアップ</strong><small>保存先：{manualDestinationLabel}</small></span><ChevronDown size={16} aria-hidden="true" /></button>
