...
import { createBackup, deleteBackup, exportBackup, fetchBackups, restoreBackup, restoreImportedBackup, updateBackupRetention, type BackupPermissions, type BackupRecord, type BackupSettings } from '../lib/backupsApi'
...
export function BackupSettingsPanel({ backupSettings, onBackupSettingsChange }: { backupSettings: BackupSettings; onBackupSettingsChange: (settings: BackupSettings) => void }) {
  ...
  const [backupPermissions, setBackupPermissions] = useState<BackupPermissions>({ canManageCreateRestore: false, canManageRetention: false })
  ...
  return <section className="panel settings-panel backup-panel">
    ...
    {!backupPermissions.canManageCreateRestore && !backupPermissions.canManageRetention && !error && <div className="backup-notice">バックアップ操作は管理者が無効にしています。</div>}
    ...
    <section className="backup-retention-settings">
      ...
      <div className="backup-retention-values">
        <div className="backup-retention-field"><div className="backup-retention-field-heading"><label htmlFor="backup-retention-b2">オンライン（B2）</label></div><span className="settings-number-input"><input id="backup-retention-b2" type="number" min={7} max={3650} value={backupSettings.retentionDays} disabled={!backupPermissions.canManageRetention || Boolean(loading)} onChange={(event) => onBackupSettingsChange({ ...backupSettings, retentionDays: Number(event.target.value) })} /><span>日</span></span></div>
        <div className="backup-retention-field"><div className="backup-retention-field-heading"><label htmlFor="backup-retention-pc">PC保存（期間）</label></div><span className="settings-number-input"><input id="backup-retention-pc" type="number" min={1} max={3650} value={backupSettings.pcRetentionDays} disabled={!backupPermissions.canManageRetention || Boolean(loading)} onChange={(event) => onBackupSettingsChange({ ...backupSettings, pcRetentionDays: Number(event.target.value) })} /><span>日</span></span></div>
      </div>
      <div className="backup-retention-destination-row"><div className="backup-retention-destination-copy"><strong>PC保存先</strong><span>選択した親フォルダ内に「Vehicle Management Backup」フォルダを作成して保存します。</span></div>{backupPermissions.canManageCreateRestore && <div className="backup-retention-destination-action"><button className="button button-secondary backup-directory-button" type="button" disabled={Boolean(loading)} onClick={() => void runChangePcBackupDirectory()}>{loading === 'pc-directory' ? '選択中…' : <><FolderOpen size={14} />PC保存先を変更</>}</button></div>}</div>
    ...
    <section className="backup-accordion">
      <div className="backup-accordion-header">
        ...
        <label className="backup-toggle"><span className="sr-only">定期バックアップを有効にする</span><input type="checkbox" checked={backupSettings.autoEnabled} disabled={!backupPermissions.canManageCreateRestore || Boolean(loading)} onChange={(event) => { onBackupSettingsChange({ ...backupSettings, autoEnabled: event.target.checked }); setPeriodicOpen(event.target.checked) }} /><span className="backup-toggle-track" aria-hidden="true"><span /></span></label>
      </div>
      {periodicOpen && <div className="backup-accordion-content" id="periodic-backup-settings">
        <div className="backup-settings-table" role="table" aria-label="定期バックアップ設定">
          ...
          <div className="backup-settings-table-row" role="row"><span className="backup-settings-table-label" role="rowheader">頻度</span><span role="cell"><select value={backupSettings.frequency} disabled={!backupPermissions.canManageCreateRestore || Boolean(loading)} onChange={(event) => onBackupSettingsChange({ ...backupSettings, frequency: event.target.value as BackupSettings['frequency'] })}><option value="daily">毎日</option><option value="weekly">毎週</option></select></span><span className="backup-table-unavailable" role="cell">未実装</span></div>
        </div>
    ...
    <section className="backup-accordion">
      ...
      {manualOpen && <div className="backup-accordion-content" id="manual-backup-settings">
        <div className="backup-manual-grid">
          <label className="backup-note-field"><span>手動バックアップのメモ（任意）</span><textarea value={manualBackupNote} maxLength={500} rows={3} disabled={!backupPermissions.canManageCreateRestore || Boolean(loading)} onChange={(event) => setManualBackupNote(event.target.value)} placeholder="例：請求書修正前" /></label>
          <label className="backup-settings-field"><span>保存先</span><select value={manualDestination} disabled={!backupPermissions.canManageCreateRestore || Boolean(loading)} onChange={(event) => setManualDestination(event.target.value as BackupSettings['destination'])}><option value="b2">オンライン（B2）</option><option value="pc">PC</option><option value="both">両方</option></select></label>
          {backupPermissions.canManageCreateRestore && <div className="backup-manual-execute"><button className="button button-secondary backup-execute-button" type="button" disabled={Boolean(loading)} onClick={() => void runManualBackup()}>{loading === 'manual-create' ? '作成中…' : <><HardDrive size={14} />バックアップを実行</>}</button></div>}
        </div>
    ...
    <section className="backup-history-section">
      ...
      {backups.length === 0 ? <div className="settings-empty backup-empty"><Archive size={26} /><span>バックアップ履歴はありません。</span></div> : <div className="backup-list">{backups.map((backup) => <div className="backup-row" key={backup.id}>
        ...
        <div className="backup-meta"><span className={`backup-retention-status${backup.keepForever ? ' is-forever' : ''}`}>{backup.keepForever ? <><ShieldCheck size={13} />永久保存</> : <><Clock3 size={13} />自動削除予定</>}</span>{(backupPermissions.canManageCreateRestore || backupPermissions.canManageRetention) && <span className="backup-actions">{backupPermissions.canManageCreateRestore && <button className="button button-secondary" type="button" disabled={Boolean(loading)} onClick={() => void runRestore(backup)}>{loading === `restore-${backup.id}` ? '復元中…' : <><RotateCcw size={14} />復元</>}</button>}{backupPermissions.canManageRetention && <><button className="button button-secondary backup-retention-button" type="button" disabled={Boolean(loading)} onClick={() => void runBackupKeepForever(backup)}>{loading === `keep-${backup.id}` ? '更新中…' : backup.keepForever ? <><ShieldCheck size={13} />保存解除</> : <><ShieldCheck size={13} />永久保存</>}</button><button className="button button-danger backup-delete-button" type="button" disabled={Boolean(loading)} onClick={() => void runDelete(backup)}>{loading === `delete-${backup.id}` ? '削除中…' : <><Trash2 size={13} />削除</>}</button></>}</span>}</div>
      </div>)}</div>}
    </section>
