  ...
  return <section className="panel settings-panel backup-panel">
    ...
    <section className="backup-retention-settings">
      ...
      <div className="backup-retention-values">
        ...
        <div className="backup-retention-field"><div className="backup-retention-field-heading"><label htmlFor="backup-retention-pc">PC保存（期間）</label></div><div className="backup-retention-pc-controls"><span className="settings-number-input"><input id="backup-retention-pc" type="number" min={1} max={3650} value={backupSettings.pcRetentionDays} disabled={!canManage || Boolean(loading)} onChange={(event) => onBackupSettingsChange({ ...backupSettings, pcRetentionDays: Number(event.target.value) })} /><span>日</span></span>{canManage && <button className="button button-secondary backup-directory-button" type="button" disabled={Boolean(loading)} onClick={() => void runChangePcBackupDirectory()}>{loading === 'pc-directory' ? '選択中…' : <><FolderOpen size={14} />PC保存先を変更</>}</button>}</div></div>
      </div>
