...
export function BackupSettingsPanel({ backupSettings, onBackupSettingsChange }: { backupSettings: BackupSettings; onBackupSettingsChange: (settings: BackupSettings) => void }) {
  ...
  return <section className="panel settings-panel backup-panel">
    ...
    <section className="backup-retention-settings">
      ...
      <div className="backup-retention-destination-row"><div className="backup-retention-destination-copy"><strong>PC保存先</strong><span>選択した親フォルダ内に「Vehicle Management Backup」フォルダを作成して保存します。</span></div>{canManage && <button className="button button-secondary backup-directory-button" type="button" disabled={Boolean(loading)} onClick={() => void runChangePcBackupDirectory()}>{loading === 'pc-directory' ? '選択中…' : <><FolderOpen size={14} />PC保存先を変更</>}</button>}</div>
