...
Gate 6Eでは、6Dの候補一覧から利用者が1件だけ選択し、ABACUSフォルダーの外側に新規JPGとして検証出力します。出力直前に現在のファイル範囲、JPEG開始・終了マーカー、最大256MiB、最大5,000万画素、WPFのJPEGデコーダー、SHA-256を再検証します。保存先が原本・作業用コピーの内部なら拒否し、既存ファイルは上書きしません。これは抽出方式の実証であり、車両との自動紐付けや一括出力ではありません。
