...
FileMaker Runtimeは、コピー側ABACUSを起動しただけでも`abx-cs-mn.ucs`の管理情報を書き換えます。再利用時は保存用原本が作成時のハッシュを維持していること、ファイル構成・サイズが同一であること、他の全ファイルが原本と一致することを検証します。`abx-cs-mn.ucs`だけはサイズ不変かつ差分4,096バイト以内に限定して許容し、それを超える変更や他ファイルの変更は拒否します。
