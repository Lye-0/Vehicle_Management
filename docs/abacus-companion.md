...
再検証に合格すると「顧客・車両・書類のグラフ型確認」が表示されます。左側で顧客を選ぶと、顧客から車両、車両から書類へ関係をたどれます。右側には選択した顧客・車両・書類の詳細と判定根拠を表示し、画面下部には未確定車両・未確定販売書類・未確定整備書類を分けて表示します。マニフェストが一意一致と判定した書類は緑色の実線、要確認・未一致・除外は点線と色で区別します。線はブロックの移動中もノードIDを基準に追従します。グラフ上のホイール操作はタブ全体のスクロールへ渡し、キャンバスの空白を左ドラッグした場合だけキャンバスを移動します。移動範囲は、表示領域内に少なくとも1つのブロックが残るよう制限します。書類カードのハンドル、または未確定トレイのカードを車両へドラッグすると、画面上だけの手動仮紐付けとして車両側へ青い点線を表示します。元の候補CSV、マニフェスト、ABACUS原本は変更せず、画面を再読込すると仮紐付けは破棄されます。

顧客一覧はグループ候補を親、統合元顧客を1段インデントした階層で表示します。通常顧客には展開用のボックスを表示せず、一覧幅と右側の余白を確保して長い顧客名・住所がスクロールバーに隠れないようにしています。同じ正規化顧客名の候補は「同名候補」として自動的にグループ化されます。顧客カードを別の顧客カードまたは候補グループへドラッグすると、名前が異なる顧客も「手動統合候補」として比較画面へ追加できます。ドラッグ中に一覧の上下端へ近づくと一覧が自動スクロールします。比較画面で電話・メール・住所などを候補ごとに選択して統合プレビューを保存できます。候補の追加・削除・グループ解消で既存のプレビューが古くなった場合は、再比較が必要になります。保存済みの統合プレビューはGate 7.6で「統合プレビューをキャンバスに仮反映」を押した場合だけ、重複を除いた車両・書類を仮想顧客ブロックへまとめて表示できます。解除すると元の候補表示へ戻り、左の顧客一覧・元の顧客ID・候補CSV・データベース・APIは変更しません。統合プレビューは画面上だけの状態であり、手動判断の永続的な証跡保存は次段階で追加します。
