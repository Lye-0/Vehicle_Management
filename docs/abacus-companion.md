...
再検証に合格すると「顧客・車両・書類のグラフ型確認」が表示されます。左側で顧客を選ぶと、顧客から車両、車両から書類へ関係をたどれます。右側には選択した顧客・車両・書類の詳細と判定根拠を表示し、画面下部には未確定車両・未確定販売書類・未確定整備書類を分けて表示します。マニフェストが一意一致と判定した書類は緑色の実線、要確認・未一致・除外は点線と色で区別します。線はブロックの移動中もノードIDを基準に追従します。同姓同名の顧客は顧客IDが異なる限り別ブロックのままです。書類を選択して「書類の紐付け先を変更」から車両を選ぶと、画面上だけの手動仮紐付けとして車両側へ青い点線を表示します。元の候補CSV、マニフェスト、ABACUS原本は変更せず、画面を再読込すると仮紐付けは破棄されます。顧客統合、項目単位の採用値選択、手動判断の証跡保存は次段階で追加します。
