...
再検証に合格すると「顧客・車両・書類のグラフ型確認」が表示されます。左側で顧客を選ぶと、顧客から車両、車両から書類へ関係をたどれます。右側には選択した顧客・車両・書類の詳細と判定根拠を表示し、画面下部には未確定車両・未確定販売書類・未確定整備書類を分けて表示します。車両右側と書類左側の丸いノードが接続点です。マニフェストが一意一致と判定した書類は緑色の実線、手動仮紐付けは青色の点線、未接続は赤色の点線で区別します。書類ブロックまたは書類ノードを別の車両へドラッグすると、画面上だけの手動仮紐付けとして接続先を変更できます。書類右上の「×」でノード接続を切ると、キャンバス上の未接続状態になります。未接続書類は車両へ接続するか、書類ブロックを未確定トレイへ移動してください。トレイへ移した書類は、元々自動確定だった場合も一時的にキャンバスから外れます。元の候補CSV、マニフェスト、ABACUS原本は変更せず、画面を再読込すると仮紐付けは破棄されます。

顧客一覧はグループ候補を親、統合元顧客を1段インデントした階層で表示します。通常顧客には展開用のボックスを表示せず、一覧幅と右側の余白を確保して長い顧客名・住所がスクロールバーに隠れないようにしています。同じ正規化顧客名の候補は統合候補として自動的にグループ化されます。顧客カードを別の顧客カードまたは候補グループへドラッグすると、名前が異なる顧客も統合候補へ追加できます。ドラッグ中に一覧の上下端へ近づくと一覧が自動スクロールします。候補グループはドラッグ直後からキャンバスへ仮想顧客として表示され、右側の「顧客情報をプレビュー」で電話・メール・住所などの採用値と統合後の顧客名を選択します。プレビュー保存後に「キャンバスを承認」を押すと、左一覧のグループが青色の承認済みになります。顧客を候補から外す場合は、子顧客ブロックを一覧下部の解除領域へドラッグします。候補が1件になった場合は通常の顧客へ戻ります。候補構成を変更するとプレビューと承認状態は自動的に解除されます。元の顧客ID・候補CSV・データベース・APIは変更しません。
