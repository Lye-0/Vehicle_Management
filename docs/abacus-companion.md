...
キャンバス承認時に未確定トレイへ残っている販売・整備書類がある場合は、種類別の件数を警告ダイアログで表示します。承認を選ぶと、その書類は今回のインポート対象から除外する状態として画面上で記録し、トレイから車両へ戻した書類は除外状態と承認状態を解除します。元の候補CSV、マニフェスト、ABACUS原本は変更しません。現在のグラフ画面は登録前の確認段階であり、除外一覧を実際の書類CSV/API登録へ引き継ぐ処理は、書類登録パッケージを追加する次段階で実装します。
