...
再検証に合格すると「顧客・車両・書類のグラフ型確認」が表示されます。左側で顧客を選ぶと、顧客から車両、車両から書類へ関係をたどれます。右側には選択した顧客・車両・書類の詳細と判定根拠を表示し、画面下部には未確定車両・未確定販売書類・未確定整備書類を分けて表示します。マニフェストが一意一致と判定した書類は緑色の実線、要確認・未一致・除外は点線と色で区別します。線はブロックの移動中もノードIDを基準に追従します。同姓同名の顧客は顧客IDが異なる限り別ブロックのままです。ブロックのドラッグは現段階では表示位置だけを変更し、候補データ、マニフェスト、CSV、ABACUS原本は変更しません。紐付け先の変更、顧客統合、項目単位の採用値選択を保存する操作は、次段階の人手確認機能で追加します。
